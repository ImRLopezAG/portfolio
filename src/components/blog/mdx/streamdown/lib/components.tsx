...
import { decodeComponentProps } from './custom-components'
	...
	({ children, className, node, ...props }: SectionProps) => {
		...
		if (isFootnotesSection) {
			...
			const isEmptyFootnote = (listItem: React.ReactNode): boolean => {
				...
				// @ts-expect-error
				const itemChildren = Array.isArray(listItem.props.children) // @ts-ignore
				...
				for (const itemChild of itemChildren) {
					...
					if (typeof itemChild === 'string') {
						...
						if (itemChild.props?.['data-footnote-backref'] !== undefined) {
							...
							// @ts-expect-error
							const grandChildren = Array.isArray(itemChild.props.children) // @ts-ignore
								? itemChild.props.children // @ts-ignore
							...
							for (const grandChild of grandChildren) {
								...
								if (
									isValidElement(grandChild) && // @ts-ignore
...
const MemoParagraph = memo<ParagraphProps>(
	({ children, className, node, ...props }: ParagraphProps) => {
		...
		const singleChild =
			validChildren.length === 1 && isValidElement(validChildren[0])
				? validChildren[0]
				: null

		if (singleChild && singleChild.type === StreamdownComponentBridge) {
			return <>{children}</>
		}
...
type StreamdownComponentProps = WithNode<
	DetailedHTMLProps<HTMLAttributes<HTMLElement>, HTMLElement>
>

type StreamdownComponentRawProps = StreamdownComponentProps & {
	'data-sd-component'?: string
	'data-sd-props'?: string
}

function StreamdownComponentBridge(rawProps: StreamdownComponentProps) {
	const registry = useContext(ComponentsRegistryContext)
	const {
		'data-sd-component': componentName,
		'data-sd-props': encodedProps,
		children,
	} = rawProps as StreamdownComponentRawProps

	if (!componentName || typeof componentName !== 'string' || !registry) {
		return null
	}

	const Component =
		registry[componentName] ?? registry[componentName.toLowerCase()]

	if (!Component) {
		return null
	}

	const parsedProps = decodeComponentProps(
...
StreamdownComponentBridge.displayName = 'StreamdownComponentBridge'
...
export const components = {
	...
	p: MemoParagraph,
	section: MemoSection,
	'streamdown-component': StreamdownComponentBridge,
