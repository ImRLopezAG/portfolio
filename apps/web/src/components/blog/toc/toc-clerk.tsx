			...
			{svg ? (
				...
				>
					<TocThumb
						...
						className='mt-(--top) h-(--height) bg-primary transition-all'
					/>
				</div>
			) : null}
...
function TOCItem({
	...
	return (
		<Primitive.TOCItem
			...
			className='prose relative py-1.5 text-muted-foreground text-sm transition-colors [overflow-wrap:anywhere] first:pt-0 last:pb-0 data-[active=true]:text-primary'
		>
			{offset !== upperOffset ? (
				<svg
				...
				>
					<title>TOC connecting line</title>
					<line
						x1={upperOffset}
						y1='0'
						x2={offset}
